...
await test.ExecuteGET<float?>("http://localhost:5000/B/2", (float? x, float? y, int status) => (x == 2f && y == null && status == 200) ? Test.OK : Test.NOK);
...
internal sealed class Test
{
    ...
    internal const string NOK = "NOK";

    private readonly HttpClient client = new();

    public async Task ExecuteGET<T>(string path, Func<T?, T?, int, string> result) =>
        await ResultPrint("GET", path, await client.GetAsync(path), result);

    public async Task ExecutePOST<T>(string path, Func<T?, T?, int, string> result) =>
        await ResultPrint("POST", path, await client.PostAsync(path, null), result);

    public async Task ExecutePUT<T>(string path, Func<T?, T?, int, string> result) =>
        await ResultPrint("PUT", path, await client.PutAsync(path, null), result);

    public async Task ExecuteDELETE<T>(string path, Func<T?, T?, int, string> result) =>
        await ResultPrint("DELETE", path, await client.DeleteAsync(path), result);
        ...
        Func<T?, T?, int, string> result)
    {
        ...
        try
        {
            Answer<T>? answer = await response.Content.ReadFromJsonAsync<Answer<T>>() ?? new Answer<T>();
